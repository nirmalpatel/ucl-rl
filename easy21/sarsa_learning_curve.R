...
for (lambda in c(0, 1)) {
  ...
  for (ep in 1:n_ep) {
    ...
    ep_error <- mean((mc_action_vfun - action_vfun) ^ 2)
    ep_errors <- c(ep_errors, ep_error)
  }
  
  sarsa_ep_errors <- ep_errors
  ...
  sarsa_action_vfuns[[length(sarsa_action_vfuns) + 1]] <- list(
    lambda = lambda,
    sarsa_ep_errors = sarsa_ep_errors,
...
tibble(tstep = 1:n_ep,
       lambda_0_err = sarsa_action_vfuns[[1]]$sarsa_ep_errors,
       lambda_1_err = sarsa_action_vfuns[[2]]$sarsa_ep_errors) %>%
